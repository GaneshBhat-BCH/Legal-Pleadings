...
,
  ...
  "prompt": "Perform a DEEP LEGAL ANALYSIS using ONLY REFERENCE_POLICIES. Review the assumptions and raw input first. Follow these steps STRICTLY:\n\nSTEP 1 - INVENTOR CHECK: Determine if the researcher is an INVENTOR/CO-INVENTOR of IP licensed/transferred to the company.\n- If YES, strictly evaluate 'Inventor_Equity_and_Licensing_Conflict_Policy'. If ALL its conditions are met, select it and STOP.\n\nSTEP 2 - GENERAL POLICY EVALUATION: If Step 1 does not yield a match, evaluate 'HMS_COI_Policy', 'PHS_COI_Policy', and 'BCH_COI_Policy'.\n- For a policy to be applicable, at least ONE of its Rules must be a 'PERFECT MATCH'.\n- A Rule is a 'PERFECT MATCH' ONLY if ALL conditions listed in that Rule are fully satisfied by the raw data.\n- If a Rule has multiple conditions (e.g., A1 has 4 conditions), ALL 4 must be satisfied. If even one is missing or false, the Rule does NOT apply.\n\nSTEP 3 - SELECTION:\n- Select the Policy Name containing the matched Restricted Rule.\n- If multiple policies match, prioritize specific rules over general ones.\n- Output ONLY the Policy Name. If no policy has a rule that strictly matches all conditions, output 'NA'."
}
